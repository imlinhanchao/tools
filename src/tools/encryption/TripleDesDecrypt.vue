...
function process() {
  try {
    ...
    if (key.value.length !== 16 && key.value.length !== 24) {
      message.warning('3DES 密钥必须为 16 或 24 个字符'); return
    }
    ...
    const decrypted = CryptoJS.TripleDES.decrypt(cipherInput, keyParsed, options)
  ...
  <div class="tool-container">
    <div class="options-row">
      <div class="option-item">
        <div class="option-label">密钥 (16或24字符)</div>
        <el-input v-model="key" placeholder="输入16或24字节密钥" style="width:220px" />
      </div>
      <div class="option-item">
