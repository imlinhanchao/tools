...
function scalaType(val: any, key: string): string {
  if (val === null) return 'Option[Any]'
  if (Array.isArray(val)) {
    if (val.length === 0) return 'List[Any]'
    return `List[${scalaType(val[0], key)}]`
  }
...
function toScalaCaseClass(obj: any, name = 'Root'): string {
  const params: string[] = []
  const nested: string[] = []

  for (const [k, v] of Object.entries(obj)) {
    const typeName = scalaType(v, k)
    params.push(`  ${k}: ${typeName}`)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toScalaCaseClass(v, toPascalCase(k)))
    } else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) {
      nested.push(toScalaCaseClass(v[0], toPascalCase(k)))
    }
  }

  const caseClass = `case class ${name}(\n${params.join(',\n')}\n)`
  return [...nested, caseClass].join('\n\n')
}
...
function transform() {
  ...
  try {
    ...
    output.value = toScalaCaseClass(data)
  } catch (e) {
