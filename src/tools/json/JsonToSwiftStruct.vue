...
function swiftType(val: any, key: string): string {
  if (val === null) return 'Any?'
  if (Array.isArray(val)) {
    if (val.length === 0) return '[Any]'
    return `[${swiftType(val[0], key)}]`
  }
...
function toSwiftStruct(obj: any, name = 'Root'): string {
  const fields: string[] = []
  const nested: string[] = []

  for (const [k, v] of Object.entries(obj)) {
    const typeName = swiftType(v, k)
    fields.push(`    let ${k}: ${typeName}`)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toSwiftStruct(v, toPascalCase(k)))
    } else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) {
      nested.push(toSwiftStruct(v[0], toPascalCase(k)))
    }
  }

  const struct = `struct ${name}: Codable {\n${fields.join('\n')}\n}`
  return [...nested, struct].join('\n\n')
}
...
function transform() {
  ...
  try {
    ...
    output.value = toSwiftStruct(data)
  } catch (e) {
