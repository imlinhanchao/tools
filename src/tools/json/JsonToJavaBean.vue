...
function javaType(val: any, key: string): string {
  ...
  if (Array.isArray(val)) {
    ...
    return `List<${javaType(val[0], key)}>`
  }
...
function toJavaBean(obj: any, name = 'Root'): string {
  ...
  for (const [k, v] of Object.entries(obj)) {
    const typeName = javaType(v, k)
    const fieldName = k
    const methodName = toPascalCase(k)
    fields.push(`    private ${typeName} ${fieldName};`)
    methods.push(`    public ${typeName} get${methodName}() { return ${fieldName}; }`)
    methods.push(`    public void set${methodName}(${typeName} ${fieldName}) { this.${fieldName} = ${fieldName}; }`)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toJavaBean(v, toPascalCase(k)))
    } else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) {
      nested.push(toJavaBean(v[0], toPascalCase(k)))
    }
  }
