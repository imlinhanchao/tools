...
function toJsonSchema(val: any, title = 'Root'): any {
  if (val === null) return { type: 'null' }
  if (Array.isArray(val)) {
    return {
      type: 'array',
      items: val.length > 0 ? toJsonSchema(val[0]) : {}
    }
  }
  if (typeof val === 'object') {
    ...
    if (title !== 'Root') schema.title = title
    return schema
  }
...
function transform() {
  ...
  try {
    ...
    const schema = { '$schema': 'http://json-schema.org/draft-07/schema#', ...toJsonSchema(data) }
