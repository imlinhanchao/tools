...
function toRustStruct(obj: any, name = 'Root'): string {
  ...
  for (const [k, v] of Object.entries(obj)) {
    const typeName = rustType(v, k)
    const snakeKey = k.replace(/([A-Z])/g, '_$1').toLowerCase()
    if (snakeKey !== k) {
      fields.push(`    #[serde(rename = "${k}")]`)
    }
    fields.push(`    pub ${snakeKey}: ${typeName},`)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toRustStruct(v, toPascalCase(k)))
    } else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) {
      nested.push(toRustStruct(v[0], toPascalCase(k)))
    }
  }
...
function transform() {
  ...
  try {
    ...
    output.value = 'use serde::{Deserialize, Serialize};\n\n' + toRustStruct(data)
  } catch (e) {
