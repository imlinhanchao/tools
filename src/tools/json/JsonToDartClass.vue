...
function dartType(val: any, key: string): string {
  if (val === null) return 'dynamic'
  if (Array.isArray(val)) {
    if (val.length === 0) return 'List<dynamic>'
    return `List<${dartType(val[0], key)}>`
  }
...
function toDartClass(obj: any, name = 'Root'): string {
  const fields: string[] = []
  const params: string[] = []
  ...
  for (const [k, v] of Object.entries(obj)) {
    const typeName = dartType(v, k)
    fields.push(`  final ${typeName} ${k};`)
    params.push(`    required this.${k},`)
    fromJsonLines.push(`      ${k}: json['${k}'] as ${typeName},`)
    toJsonLines.push(`      '${k}': ${k},`)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toDartClass(v, toPascalCase(k)))
    } else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) {
      nested.push(toDartClass(v[0], toPascalCase(k)))
    }
  }
