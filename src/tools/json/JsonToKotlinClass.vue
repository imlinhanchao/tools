...
function kotlinType(val: any, key: string): string {
  if (val === null) return 'Any?'
  if (Array.isArray(val)) {
    if (val.length === 0) return 'List<Any>'
    return `List<${kotlinType(val[0], key)}>`
  }
...
function toKotlinClass(obj: any, name = 'Root'): string {
  const params: string[] = []
  const nested: string[] = []

  for (const [k, v] of Object.entries(obj)) {
    const typeName = kotlinType(v, k)
    params.push(`    val ${k}: ${typeName}`)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toKotlinClass(v, toPascalCase(k)))
    } else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) {
      nested.push(toKotlinClass(v[0], toPascalCase(k)))
    }
  }

  const dataClass = `data class ${name}(\n${params.join(',\n')}\n)`
  return [...nested, dataClass].join('\n\n')
}
...
function transform() {
  ...
  try {
    ...
    output.value = toKotlinClass(data)
  } catch (e) {
