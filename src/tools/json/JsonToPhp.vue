...
function toPhpClass(obj: any, name = 'Root'): string {
  const props: string[] = []
  ...
  for (const [k, v] of Object.entries(obj)) {
    const t = phpType(v)
    props.push(`    private ${t} $${k};`)
    const methodName = toPascalCase(k)
    getters.push(`    public function get${methodName}(): ${t} { return $this->${k}; }`)
    setters.push(`    public function set${methodName}(${t} $${k}): void { $this->${k} = $${k}; }`)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toPhpClass(v, toPascalCase(k)))
    }
  }
