...
function toGoStruct(obj: any, name = 'Root'): string {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    const typeName = goType(v, k)
    fields.push(`\t${fieldName} ${typeName} \`json:"${k}"\``)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toGoStruct(v, toPascalCase(k)))
    } else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) {
      nested.push(toGoStruct(v[0], toPascalCase(k)))
    }
  }
