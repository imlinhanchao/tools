...
function toProperties(obj: any, prefix = ''): string[] {
  const lines: string[] = []
  for (const [k, v] of Object.entries(obj)) {
    const key = prefix ? `${prefix}.${k}` : k
    if (v !== null && typeof v === 'object' && !Array.isArray(v)) {
      lines.push(...toProperties(v as any, key))
    } else if (Array.isArray(v)) {
      v.forEach((item, i) => {
        if (typeof item === 'object' && item !== null) {
          lines.push(...toProperties(item, `${key}[${i}]`))
        } else {
          lines.push(`${key}[${i}]=${item}`)
        }
      })
    } else {
      lines.push(`${key}=${v ?? ''}`)
...
function transform() {
  ...
  try {
    ...
    output.value = toProperties(data).join('\n')
  } catch (e) {
