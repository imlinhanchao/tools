...
function csharpType(val: any, key: string): string {
  if (val === null) return 'object'
  if (Array.isArray(val)) {
    if (val.length === 0) return 'List<object>'
    return `List<${csharpType(val[0], key)}>`
  }
...
function toCsharpClass(obj: any, name = 'Root'): string {
  const props: string[] = []
  const nested: string[] = []

  for (const [k, v] of Object.entries(obj)) {
    const typeName = csharpType(v, k)
    const propName = toPascalCase(k)
    props.push(`    [JsonProperty("${k}")]`)
    props.push(`    public ${typeName} ${propName} { get; set; }`)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') {
      nested.push(toCsharpClass(v, toPascalCase(k)))
    } else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) {
      nested.push(toCsharpClass(v[0], toPascalCase(k)))
    }
  }
...
function transform() {
  ...
  try {
    ...
    output.value = 'using Newtonsoft.Json;\nusing System.Collections.Generic;\n\n' + toCsharpClass(data)
  } catch (e) {
