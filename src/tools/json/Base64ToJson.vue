...
function transform() {
  ...
  try {
    const json = new TextDecoder().decode(Uint8Array.from(atob(input.value.trim()), c => c.charCodeAt(0)))
...
function loadExample() {
  input.value = btoa(new TextEncoder().encode(EXAMPLE_JSON).reduce((data, byte) => data + String.fromCharCode(byte), ''))
  transform()
}
  ...
  <div class="tool-container">
    <div class="io-grid">
      <div class="io-panel">
        ...
        <el-input type="textarea" v-model="input" :rows="15" placeholder="请输入 Base64 编码..." @change="transform" />
      </div>
      <div class="io-panel">
        <div class="panel-header">
          <span>JSON 输出</span>
