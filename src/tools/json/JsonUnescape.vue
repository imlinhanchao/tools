...
function transform() {
  ...
  try {
    ...
    if (typeof val !== 'string') throw new Error('输入不是转义字符串')
    output.value = val
  } catch {
    try {
    ...
    <div class="io-grid">
      <div class="io-panel">
        <div class="panel-header">
          <span>转义 JSON 字符串</span>
          <div class="btn-group">
            <el-button size="small" @click="loadExample">示例</el-button>
            <el-button size="small" @click="input = ''; output = ''; error = ''">清除</el-button>
          </div>
        </div>
        <el-input type="textarea" v-model="input" :rows="15" placeholder="请输入转义后的JSON字符串..." @change="transform" />
      </div>
      <div class="io-panel">
        <div class="panel-header">
          <span>还原后</span>
