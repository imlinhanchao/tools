...
const example = `function greet(name){const greeting='Hello, '+name+'!';console.log(greeting);return greeting;}const numbers=[1,2,3,4,5];const doubled=numbers.map(function(n){return n*2;});if(doubled.length>0){console.log('Doubled:',doubled);}`

function transform() {
  error.value = ''
  if (!input.value.trim()) { output.value = ''; return }
  try {
    output.value = jsBeautify(input.value, {
  ...
  <div class="tool-container">
    ...
    <div class="io-grid">
      <div class="io-panel">
        ...
        <el-input type="textarea" v-model="input" :rows="14" placeholder="请输入JavaScript代码..." @change="transform" />
      </div>
      <div class="io-panel">
        <div class="panel-header">
          <span>格式化结果</span>
