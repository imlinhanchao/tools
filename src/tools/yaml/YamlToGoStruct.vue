...
function goType(val: any, key: string): string {
  if (val === null) return 'interface{}'
  if (Array.isArray(val)) { if (val.length === 0) return '[]interface{}'; return `[]${goType(val[0], key)}` }
...
function toGoStruct(obj: any, name = 'Root'): string {
  const fields: string[] = []
  const nested: string[] = []
  for (const [k, v] of Object.entries(obj as object)) {
    const fn = toPascalCase(k); const tn = goType(v, k)
    fields.push(`\t${fn} ${tn} \`json:"${k}"\``)
    if (v !== null && !Array.isArray(v) && typeof v === 'object') nested.push(toGoStruct(v, toPascalCase(k)))
    else if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object' && v[0] !== null) nested.push(toGoStruct(v[0], toPascalCase(k)))
  }
