...
function jsonToXml(obj: any, tagName: string, indent = 0): string {
  const pad = '  '.repeat(indent)
  if (obj === null || obj === undefined) return `${pad}<${tagName}/>`
  if (typeof obj !== 'object') return `${pad}<${tagName}>${String(obj)}</${tagName}>`
  if (Array.isArray(obj)) return obj.map(item => jsonToXml(item, tagName, indent)).join('\n')
