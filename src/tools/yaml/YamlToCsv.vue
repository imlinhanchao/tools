...
function transform() {
  ...
  try {
    ...
    if (!rows.length || typeof rows[0] !== 'object') throw new Error('YAML must be an object or array of objects')
    const headers = Array.from(new Set(rows.flatMap(r => Object.keys(flattenObject(r)))))
    ...
    output.value = [headers.join(','), ...csvRows].join('\n')
  } catch (e: any) {
